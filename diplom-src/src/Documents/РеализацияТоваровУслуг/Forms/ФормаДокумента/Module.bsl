...
&НаСервере
Процедура ДобавитьКнопкуЗаполнить()
	
	КомандаСформироватьТаблицу = Команды.Добавить("Заполнить");
	КомандаСформироватьТаблицу.Действие = "ВыполнитьКомандуЗаполнить";
	КомандаСформироватьТаблицу.Заголовок = "Заполнить";
	КнопкаСформироватьТаблицу = Элементы.Добавить("КнопкаСформироватьТаблицу", Тип("КнопкаФормы"));
...
&НаКлиенте
Процедура ВыполнитьКомандуЗаполнить(Команда)
	...
	Если Объект.Договор.ВидДоговора = ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.ВКМ_АбонентскоеОбслуживание") Тогда
		ЗаполнитьДокумент();
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю("Для заполнения необходимо выбрать договор абонентского обслуживания");
	КонецЕсли;
