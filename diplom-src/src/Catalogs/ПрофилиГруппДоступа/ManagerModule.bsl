...
Функция ИдентификаторПрофиляСпециалист() Экспорт
	
	Возврат Специалист.УникальныйИдентификатор();
	
КонецФункции

Функция ИдентификаторПрофиляМенеджер() Экспорт
	
	Возврат Менеджер.УникальныйИдентификатор();
	
КонецФункции

Функция ИдентификаторПрофиляБухгалтерИТФирмы() Экспорт
	
	Возврат БухгалтерИТФирмы.УникальныйИдентификатор();
	
КонецФункции

Функция ИдентификаторПрофиляКадровичРасчетчик() Экспорт
	
	Возврат КадровикРасчетчик.УникальныйИдентификатор();
...
Функция ЗаполненныеПоставляемыеПрофили()
	...
	ОписаниеПрофиляКадровикРасчетчик = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ЗаполнитьПрофильКадровикРасчетчик(ОписаниеПрофиляКадровикРасчетчик);
	ОписанияПрофилей.Добавить(ОписаниеПрофиляКадровикРасчетчик);
...
Процедура ЗаполнитьПрофильСпециалист(ОписаниеПрофиляСпециалист, КромеОписания = Ложь)
	...
	Иначе
		Описание =
			НСтр("ru = 'Предназначен для:
			           |- ввода документа Обслуживание клиента'");
	КонецЕсли;
	...
	ОписаниеПрофиляСпециалист.Идентификатор = ИдентификаторПрофиляСпециалист();
	...
	ОписаниеПрофиляСпециалист.Роли.Добавить("ВКМ_РаботаСОтчетомВыработкаСпециалистов");
	ОписаниеПрофиляСпециалист.Роли.Добавить("ЗапускТонкогоКлиента");
	ОписаниеПрофиляСпециалист.Роли.Добавить("ЗапускВебКлиента");
	ОписаниеПрофиляСпециалист.Роли.Добавить("ЗапускТолстогоКлиента");
...
Процедура ЗаполнитьПрофильМенеджер(ОписаниеПрофиляМенеджер, КромеОписания = Ложь)
	...
	ОписаниеПрофиляМенеджер.Идентификатор = ИдентификаторПрофиляМенеджер();
	...
	ОписаниеПрофиляМенеджер.Роли.Добавить("ЗапускТонкогоКлиента");
	ОписаниеПрофиляМенеджер.Роли.Добавить("ЗапускВебКлиента");
	ОписаниеПрофиляМенеджер.Роли.Добавить("ЗапускТолстогоКлиента");
...
Процедура ЗаполнитьПрофильБухгалтерИТФирмы(ОписаниеПрофиляБухгалтерИТФирмы, КромеОписания = Ложь)
	...
	Иначе
		Описание =
			НСтр("ru = 'Предназначен для:
			           |- работы с обработкой Массовое создание актов'");
	КонецЕсли;
	...
	ОписаниеПрофиляБухгалтерИТФирмы.Идентификатор = ИдентификаторПрофиляБухгалтерИТФирмы();
	ОписаниеПрофиляБухгалтерИТФирмы.Наименование  = НСтр("ru = 'Бухгалтер ИТ фирмы'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ОписаниеПрофиляБухгалтерИТФирмы.Роли.Добавить("ВКМ_РаботаСОтчетомАнализВыставленныхАктов");
	ОписаниеПрофиляБухгалтерИТФирмы.Роли.Добавить("ЗапускТонкогоКлиента");
	ОписаниеПрофиляБухгалтерИТФирмы.Роли.Добавить("ЗапускВебКлиента");
	ОписаниеПрофиляБухгалтерИТФирмы.Роли.Добавить("ЗапускТолстогоКлиента");
	ОписаниеПрофиляБухгалтерИТФирмы.Описание = Описание;
	
КонецПроцедуры

Процедура ЗаполнитьПрофильКадровикРасчетчик(ОписаниеПрофиляКадровикРасчетчик, КромеОписания = Ложь)
	
	Если КромеОписания Тогда
		Описание = ОписаниеПрофиляКадровикРасчетчик.Описание;
	Иначе
		Описание =
			НСтр("ru = 'Предназначен для:
			           |- работы с документами ВКМ_НачислениеЗарплаты;
			           |- работы с документами ВКМ_НачислениеФиксированнойПремии;
			           |- работы с документами ВКМ_ВыплатаЗарплаты;
			           |- работы с документами ВКМ_ГрафикОтпусков'");
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(ОписаниеПрофиляКадровикРасчетчик,
		УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа());
	
	ОписаниеПрофиляКадровикРасчетчик.Имя           = "КадровикРасчетчик";
	ОписаниеПрофиляКадровикРасчетчик.Идентификатор = ИдентификаторПрофиляКадровичРасчетчик();
	ОписаниеПрофиляКадровикРасчетчик.Наименование  = НСтр("ru = 'Кадровик-расчетчик'", ОбщегоНазначения.КодОсновногоЯзыка());
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("БазовыеПраваБСП");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ВКМ_ДобавлениеИзменениеНачисленияЗарплаты");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ВКМ_ДобавлениеИзменениеВыплатыЗарплаты");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ВКМ_ДобавлениеИзменениеНачисленияФиксированнойПремии");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ВКМ_ДобавлениеИзменениеГрафиковОтпусков");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ВКМ_РаботаСОтчетомВыработкаСпециалистов");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ВКМ_РаботаСОтчетомРасчетыССотрудниками");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ВКМ_РаботаСОтчетомРасходЗапланированныхОтпусков");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ЗапускТонкогоКлиента");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ЗапускВебКлиента");
	ОписаниеПрофиляКадровикРасчетчик.Роли.Добавить("ЗапускТолстогоКлиента");
	ОписаниеПрофиляКадровикРасчетчик.Описание = Описание;
...
Процедура ЗаполнитьПрофилиТП() Экспорт
	
	ПрофильСпециалист();
	ПрофильМенеджер();
	ПрофильБухгалтерИТФирмы();
	ПрофильКадровикРасчетчик();
	
КонецПроцедуры

Процедура ПрофильСпециалист()
		
	МассивРолей = ПолучитьМассивСтандартныхРолей();
	
	МассивРолей.Добавить("ВКМ_ДобавлениеИзменениеОбслуживаниеКлиента");
	МассивРолей.Добавить("ВКМ_РаботаСОтчетомВыработкаСпециалистов");
	
	ДобавитьРолиВПрофиль(Справочники.ПрофилиГруппДоступа.Специалист, МассивРолей);
	
КонецПроцедуры

Процедура ПрофильМенеджер()
		
	МассивРолей = ПолучитьМассивСтандартныхРолей();
	
	МассивРолей.Добавить("ВКМ_ДобавлениеИзменениеОбслуживаниеКлиента");
	
	ДобавитьРолиВПрофиль(Справочники.ПрофилиГруппДоступа.Менеджер, МассивРолей);
	
КонецПроцедуры

Процедура ПрофильБухгалтерИТФирмы()
	
	МассивРолей = ПолучитьМассивСтандартныхРолей();
	
	МассивРолей.Добавить("ВКМ_ИспользованиеОбработкиМассовоеСозданиеАктов");
	МассивРолей.Добавить("ВКМ_РаботаСОтчетомАнализВыставленныхАктов");
	
	ДобавитьРолиВПрофиль(Справочники.ПрофилиГруппДоступа.БухгалтерИТФирмы, МассивРолей);
	
КонецПроцедуры

Процедура ПрофильКадровикРасчетчик()
	
	МассивРолей = ПолучитьМассивСтандартныхРолей();
	
	МассивРолей.Добавить("ВКМ_ДобавлениеИзменениеНачисленияЗарплаты");
	МассивРолей.Добавить("ВКМ_ДобавлениеИзменениеВыплатыЗарплаты");
	МассивРолей.Добавить("ВКМ_ДобавлениеИзменениеНачисленияФиксированнойПремии");
	МассивРолей.Добавить("ВКМ_ДобавлениеИзменениеГрафиковОтпусков");
	МассивРолей.Добавить("ВКМ_РаботаСОтчетомВыработкаСпециалистов");
	МассивРолей.Добавить("ВКМ_РаботаСОтчетомРасчетыССотрудниками");
	МассивРолей.Добавить("ВКМ_РаботаСОтчетомРасходЗапланированныхОтпусков");
	
	ДобавитьРолиВПрофиль(Справочники.ПрофилиГруппДоступа.КадровикРасчетчик, МассивРолей);
	
КонецПроцедуры

Процедура ДобавитьРолиВПрофиль(Профиль, МассивРолей)
	...
	Если ПрофильОбъект.Назначение.Количество() = 0 Тогда 
		ПрофильОбъект.Назначение.Добавить();  
	КонецЕсли;	
...
Функция ПолучитьМассивСтандартныхРолей()
	
	МассивРолей = Новый Массив;
	МассивРолей.Добавить("БазовыеПраваБСП");
	МассивРолей.Добавить("ВКМ_ПодсистемаДобавленныеОбъекты");
	МассивРолей.Добавить("ВыводНаПринтерФайлБуферОбмена");
	МассивРолей.Добавить("ДобавлениеИзменениеПапокИФайлов");
	МассивРолей.Добавить("ЗапускAutomation");
	МассивРолей.Добавить("ЗапускВебКлиента");
	МассивРолей.Добавить("ЗапускВнешнегоСоединения");
	МассивРолей.Добавить("ЗапускМобильногоКлиента");
	МассивРолей.Добавить("ЗапускТолстогоКлиента");
	МассивРолей.Добавить("ЗапускТонкогоКлиента");
	МассивРолей.Добавить("ПросмотрСвязанныеДокументы"); 
	МассивРолей.Добавить("УдаленныйДоступБазоваяФункциональность");
	МассивРолей.Добавить("ЧтениеВариантовОтчетов");
	Возврат МассивРолей;
